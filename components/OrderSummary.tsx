        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          ...
          <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
            {state.items.map((item) => (
              <div key={item._id} className="flex items-center gap-3">
                ...
                <div className="text-sm font-medium text-gray-900">Le{(item.price * item.quantity).toFixed(2)}</div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 border-t border-gray-200 pt-4 space-y-2 text-sm">
            <div className="flex justify-between text-gray-600">
              <span>Subtotal</span>
              <span>Le{totals.subtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>Shipping</span>
              <span>{totals.shippingCost === 0 ? 'Free' : `Le${totals.shippingCost.toFixed(2)}`}</span>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>Tax</span>
              <span>Le{totals.tax.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-gray-900 font-semibold text-base">
              <span>Total</span>
              <span>Le{totals.total.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </aside>
